...
### Visual comparison (MT-002 across conditions)
`experiments/results/qualitative/fig_mt002_conditions.png` (generated by
`scripts/generate_qualitative.py`): original vs binarized vs full_restoration.
Thin strokes (flourish loop, headline rules, character joints) visibly erode
under thresholding while surviving in grayscale — the CER jump 0.099 → 0.176 →
0.242 has a visible cause, not just a numeric one. This is the mechanism behind
the binarization finding in §15.
...
### Verification mechanics test (2026-09-17, live API E2E — workflow proof, not scholarship)
AI transcription:        real Colab output for MT-007 (66 chars, via POST /api/manuscripts)
Human-corrected text:    "live verified text" (deliberate test string, NOT a scholarly correction)
Nature of corrections:   none claimed — this run proved the verify endpoint writes verified_transcription + verified_at, flips status to verified, preserves ai_transcription, and surfaces the record in search. No team member reads Modi well enough to contribute a genuine correction example; that remains open.
